...
class CashfreePopupVC: UIViewController {
    ...
    func configView() {
        ...
        phoneTextField.tintColor = .PrimaryColor
    ...
    // Set User Data
    func setUserData() {
        self.nameTextField.text = AppInstance.shared.userProfileSettings?.fullname ?? ""
        self.emailTextField.text = AppInstance.shared.userProfileSettings?.email ?? ""
        self.phoneTextField.text = AppInstance.shared.userProfileSettings?.phone_number ?? ""
    }
}
...
extension CashfreePopupVC: UITextFieldDelegate {
    func textFieldDidBeginEditing(_ textField: UITextField) {
        if textField == nameTextField {
            nameTextView.borderColorV = .PrimaryColor
        } else if textField == emailTextField {
            emailTextView.borderColorV = .PrimaryColor
        } else if textField == phoneTextField {
            phoneTextView.borderColorV = .PrimaryColor
        }
    }
